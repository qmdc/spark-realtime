...
import com.qiandao.gmall.realtime.bean.{DauInfo, PageLog}
import com.qiandao.gmall.realtime.util.MyEsUtils.searchField
import com.qiandao.gmall.realtime.util.{MyBeanUtils, MyEsUtils, MyKafkaUtils, MyOffsetsUtils, MyRedisUtils}
...
object DwdDauApp {
    ...
    dauInfoDStream.foreachRDD(
      rdd => {
        rdd.foreachPartition(
          dauInfoItem => {
            val docsScala: List[(String, DauInfo)] = dauInfoItem.map(dauInfo => (dauInfo.mid, dauInfo)).toList
            import scala.collection.JavaConverters._
            val docs: util.List[(String, DauInfo)] = docsScala.asJava
            if (docs.size > 0) {
              2
              // 索引名
  ...
  def revertState(): Unit = {
    ...
    val list: util.List[String] = MyEsUtils.searchField(indexName, fieldName)
    val mids: List[String] = list.toList
