...
import com.qiandao.gmall.realtime.bean.{OrderDetail, OrderInfo, OrderWide}
import com.qiandao.gmall.realtime.util.{MyEsUtils, MyKafkaUtils, MyOffsetsUtils, MyRedisUtils}
    ...
    orderWideDStream.foreachRDD(
      rdd => {
        rdd.foreachPartition(
          orderWideIter => {
            val orderWidesScala: List[(String, OrderWide)] = orderWideIter.map(orderWide => (orderWide.detail_id.toString, orderWide)).toList

            import scala.collection.JavaConverters._
            val orderWides: util.List[(String, OrderWide)] = orderWidesScala.asJava
            ...
            if (orderWides.size > 0) {
              ...
              //写入到ES
              MyEsUtils.bulkSaveOrder(indexName, orderWides)
            }
          }
        )
        //提交offsets
